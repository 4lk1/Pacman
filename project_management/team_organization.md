...
This activity was carried out by aleka and gkordhak; the split below
documents how responsibilities were handled:
...
- Ambiguities were resolved by re-reading the subject first, then by
  choosing the interpretation that follows the explicit text most directly
  (examples: `pacgum` interpreted as a percentage; timeout behavior chosen
  from the options the subject lists).
