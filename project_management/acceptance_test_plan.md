...
| 3 | Last-pacgum walk in tests did not account for level transition | Test helper now detects mid-walk level completion. |
...
| 6 | Headless tests posted events before `pygame.init()` | Create the app before posting events. |
| 7 | `event.unicode` was missing on constructed test events | Use `getattr(event, "unicode", "")`. |
